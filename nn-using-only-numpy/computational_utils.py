...
class MultiplyGate:
    def forward(self,W, x):
        # What's dot product between two matrices?
        return np.dot(W, x)

    # Do not call this one. It's already obsolete. Kept it here for unit tests.
    def backwardObsolete(self, W, x, dz):
        # I don't understand the meaning of this...

        # NOTES: some huge performance issue here. Probably should optimize it. Took about 90% of the running time in the following two statements.
        dW = np.asarray(np.dot(np.transpose(np.asmatrix(dz)), np.asmatrix(x)))
        dx = np.dot(np.transpose(W), dz)

        '''
        logger.debug('shape W=%s, shape dw=%s, shape x=%s, shape dx=%s, shape dz=%s' %
            (W.shape, dW.shape, x.shape, dx.shape, dz.shape))
        logger.debug('W=%s, dw=%s, x=%s, dx=%s, dz=%s' %
            (W, dW, x, dx, dz))
        '''
    ...
    def backward(self, W, x, dz):
        '''
        logger.debug('### shape dz=%s, shape x = %s, shape W=%s' % (dz.shape, x.shape, W.shape))
        logger.debug('shape W=%s, shape dw=%s, shape x=%s, shape dx=%s, shape dz=%s' %
            (W.shape, dW.shape, x.shape, dx.shape, dz.shape))
        logger.debug('W=%s, dw=%s, x=%s, dx=%s, dz=%s' %
            (W, dW, x, dx, dz))
        ...
        '''
        Types of those inputs during backpropagation:
        2023-03-23 07:39:35,815 - rnn_using_only_np - DEBUG - ###type of W: <class 'numpy.ndarray'>, dW:<class 'numpy.ndarray'>, dz: <class 'numpy.ndarray'>, x: <class 'numpy.ndarray'>, dx: <class 'numpy.ndarray'>
        '''

        return dW, dx


    def backwardPreAllocated(self, W, x, dz, pre_allocated_dW, pre_allocated_dx):
        np.outer(dz, x, out=pre_allocated_dW)
        np.dot(W.T, dz, out=pre_allocated_dx)
